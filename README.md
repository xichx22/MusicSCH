...
그래서 아빠 화면에 **링크로 노래 넣기** 를 뒀다. 스포티파이 앱에서
곡 → 공유 → 링크 복사 해서 붙여넣으면 **검색을 한 번도 쓰지 않고**
그 조합에 노래가 붙는다. 제목과 앨범 그림은 `GET /tracks/{id}` 로
가져오는데(다른 바구니라 살아 있을 수 있다), 그것마저 막혀 있으면
카드 이름을 제목으로 쓰고 그냥 넣는다.
