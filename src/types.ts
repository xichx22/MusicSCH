...
export interface Song {
  ...
  combo?: string
  /**
   * 아직 어느 주제 카드에 넣을지 못 정한 곡. 값은 캐릭터 id.
   * 앨범을 통째로 가져오면 제목만으로 주제를 못 정하는 곡이 나온다.
   * 버리지 않고 남겨두었다가, 이 곡들을 보고 카드를 만든다.
   */
  needsTopic?: string
  /**
...
export interface Settings {
   ...
   * 앱을 어떻게 쓸지.
   *
   * search = 카드를 눌렀을 때 스포티파이에서 찾아본다. 새 노래를
   *          만날 수 있지만 할당량을 쓰고, 엉뚱한 게 나올 수도 있다.
   * list   = 미리 넣어둔 노래만 쓴다. 카드도 그 목록에서 만든다.
   *          검색을 아예 안 하니 할당량과 무관하고, 누르면 반드시 나온다.
   */
  mode: 'search' | 'list'
  /** search 모드에서 검색을 잠시 끄기 */
